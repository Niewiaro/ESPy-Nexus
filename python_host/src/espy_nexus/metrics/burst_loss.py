...
class BurstLossResult:
    ...
    max_blackout_time_ms: float
    # key: burst length, value: list of starting packet IDs for that burst length
    burst_events: dict[int, list[int]]


def calculate_burst_loss(
    received_ids: pd.Series, total_sent: int, expected_iat_us: float
) -> BurstLossResult:
    """
    ...
    max_blackout_time_ms = (max_len * expected_iat_us) / 1000.0 if max_len > 0 else 0.0

    return BurstLossResult(
        total_burst_events=total_events,
        max_burst_length=max_len,
        max_blackout_time_ms=max_blackout_time_ms,
...
def print_burst_loss_result(result: BurstLossResult) -> None:
    ...
    print(f"Max Blackout Time (ms): {result.max_blackout_time_ms:.2f}")
