...
Zbudowany system umożliwił przeprowadzenie powtarzalnych eksperymentów z rzeczywistym mikrokontrolerem, a więc z uwzględnieniem ograniczeń procesora, pamięci, sterowników i stosów komunikacyjnych, których nie można w pełni odtworzyć w symulacji programowej.
Zastosowanie zewnętrznej pamięci PSRAM pozwoliło na buforowanie dużej liczby rekordów, natomiast binarna akwizycja ograniczyła narzut przesyłania wyników z ESP32 do komputera.

Cel analityczny również został zrealizowany.
...
Nie można natomiast uznać, że system zapewnia deterministyczne źródło czasu rzeczywistego.
...
Ograniczenie to zostało zidentyfikowane i uwzględnione w interpretacji wyników, dlatego nie podważa realizacji celu projektowego, ale wyznacza zakres wiarygodnych wniosków.
...
\begin{enumerate}
    ...
    \item Interfejs szeregowy może stanowić użyteczną linię bazową, lecz jego przepustowość szybko ogranicza wzrost rozmiaru ładunku.
          Dla \texttt{SERIAL\_BIN} z ładunkiem 4~B PDR wynosił 100\% do 3900~Hz, natomiast przy 5700~Hz spadł do 93,4\%.
    ...
    \item Topologia sieciowa istotnie wpływa na wyniki.
          ...
          Przy wysokich częstotliwościach ranking mógł się odwracać; dla ładunku 16~B i zakresu 9000--10.000~Hz średni PDR wyniósł około 92,6\% dla \texttt{AP\_PC} oraz 27,5\% dla \texttt{AP\_ESP}.
...
Wyniki badań prowadzą do następujących zaleceń projektowych:
...
\begin{itemize}
    \item W przypadku zamkniętej pętli sterowania, w której najważniejsze są aktualność danych i ograniczenie opóźnienia, należy w pierwszej kolejności rozważyć UDP.
          Protokół powinien być uzupełniony przez warstwę aplikacyjną zawierającą identyfikatory, kontrolę aktualności, ewentualne odrzucanie przestarzałych ramek i reakcję na utratę danych.
